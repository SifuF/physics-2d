...
namespace p2d {
	...
	public:
		...
		void setTempPosition(const Vec2f& v) {
			tempPos = v;
		}
		...
		void setTempVelocity(const Vec2f& v) {
			tempVel = v;
		}

		void setAcceleration(const Vec2f& v) {
			acc = v;
		}

		void applyAcceleration(const Vec2f& v) {
			acc = acc + v;
		}

		void setForce(const Vec2f& v) {
			force = v;
		}

		void applyForce(const Vec2f& v) {
			force = force + v;
		}
		...
		void setThetaDot(float t) {
			thetaDot = t;
		}

		void setThetaDotDot(float t) {
			thetaDotDot = t;
		}

		void setIthetaDotDot(float t) {
			IthetaDotDot = t;
		}

		void applyThetaDotDot(float t) {
			thetaDotDot = thetaDotDot + t;
		}

		void applyIthetaDotDot(float t) {
			IthetaDotDot = IthetaDotDot + t;
		}
		...
		float getThetaDotDot() {
			return thetaDotDot;
		}

		float getIthetaDotDot() {
			return IthetaDotDot;
		}
	...
	public:												 // -av + -bv^2
		...
		void update(double d) {
			...
			for (Body* o : m_Body) {		//Source Body
				...
				if (o->isStatic()) {		//Static Bodys can be targets (collided with and resolved from), but are not resolved themselves.
					continue;
				}
			...
			for (Body* o : m_Body) {
				...
				//clamp velocity to zero for slow very Bodys
