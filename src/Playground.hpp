...
private:
	...
	sf::Texture texture;
	std::optional<sf::Sprite> shape_6;
...
public:
	Playground() {
		...
		window.create(sf::VideoMode({ screenWidth, screenHeight }), "SifuF Playground");
		...
		if(!texture.loadFromFile("gfx/image.png")) {
		    throw std::runtime_error("Cannot open wav file");
	    }
		shape_6.emplace(texture);
		sf::FloatRect rec = shape_6->getGlobalBounds();
		shape_6->setOrigin({ rec.size.x / 2.0f, rec.size.y / 2.0f });
		shape_6->setColor(sf::Color::White);
		body_6.create(rec.size.x, rec.size.y, 600.0f, 600.0f, 0.5f, 0.70f);
		physics.add(&body_6);
		mouse.add(&body_6);
